...
setup(StorageUpLo upLo, int n, int max_nnz, int indexBase,
      HeCRSMatrix<CRS> &A, HeMatrix<FS> &A_)
{
    typedef typename HeCRSMatrix<CRS>::ElementType  ElementType;
...
template <typename CRS, typename FS>
void
mv(int n, int max_nnz, const HeCRSMatrix<CRS> &A, const HeMatrix<FS> &A_)
{
    typedef typename HeCRSMatrix<CRS>::ElementType  ElementType;
    ...
    for (int j=1; j<=n; ++j) {
        x(j) = complex<double>(rand() % 10, rand() % 10);
    }
    ...
    for (int test=1; test<=20; ++test) {
        ...
        //
        for (int i=1; i<=n; ++i) {
            y(i) = complex<double>(rand() % 1000, rand() % 1000);
        }
        y_ = y;
...
main()
{
    ...
    for (int run=1; run<=30; ++run) {
        ...
        for (int indexBase=-3; indexBase<=3; ++indexBase) {
            ...
            //
            {
                HeCRSMatrix<CRS<complex<double> > >       A;
