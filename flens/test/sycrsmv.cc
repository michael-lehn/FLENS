...
setup(StorageUpLo upLo, int n, int max_nnz, int indexBase,
      SyCRSMatrix<CRS> &A, SyMatrix<FS> &A_)
{
    typedef typename SyCRSMatrix<CRS>::ElementType  ElementType;
...
template <typename CRS, typename FS>
void
mv(int n, int max_nnz, const SyCRSMatrix<CRS> &A, const SyMatrix<FS> &A_)
{
    typedef typename SyCRSMatrix<CRS>::ElementType  ElementType;
